@extends('backend.app')
@section('title','Brand-Banner-Lists')
@section('content')
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('brand-banners.create')}}"> Add Brand-Banner</a></li>
                        ...
                        <table class="table text-center" id="table">
                            ...
                            @foreach($banners as $key=>$item)
                                <tr id="item-{{$item->id}}">
                                    ...
                                    <td>
                                        {{-- @if(auth()->user()->can('edit-brand-banner')) --}}
                                        <a href="{{route('brand-banners.edit', $item->slug)}}" class="edit-modal btn btn-info btn-circle btn-sm"
                                           data-info="">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- @endif --}}
                                        {{-- @if(auth()->user()->can('delete-brand-banner')) --}}
                                        <a href="javascript:void(0)" class="delete-brand-banners btn btn-danger btn-circle btn-sm"
                                           data-type="{{$item->id}}">
...
@section('js_script')
            <script>
                $(document).ready( function () {
                    ...
                    $('#table').on('click','.delete-brand-banners',function(event){
                        event.preventDefault();
                        $object = $(this);
                        var id  = $(this).attr('data-type');
                        var url = baseUrl+"/brand-banners/"+id;
                        swal({
                            title: 'Are you sure?',
                    ...
                    $("#table").on("click", "#change-status", function () {
                        ...
                        swal({
                            ...
                            if (result.value) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ route('brand-banners.change-status') }}",
                                    data: {
                                        'id': id,
                                    },
                                    headers: {
