                            ...
                            @foreach($discounts as $key=>$item)
                                <tr id="item-{{$item->id}}">
                                    ...
                                    <td>
                                        ...
                                        <a href="{{route('discounts.edit', $item->slug)}}" class="edit-modal btn btn-info btn-circle btn-sm"
                                           data-info="">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- @endif --}}
                                        {{-- @if(auth()->user()->can('delete-discount')) --}}
                                        <a href="javascript:void(0)" class="delete-discounts btn btn-danger btn-circle btn-sm"
                                           data-type="{{$item->id}}">
...
@section('js_script')
            <script>
                $(document).ready( function () {
                    ...
                    $('#table').on('click','.delete-discounts',function(event){
                        event.preventDefault();
                        $object = $(this);
                        var id  = $(this).attr('data-type');
                        var url = baseUrl+"/discounts/"+id;
                        swal({
                            title: 'Are you sure?',
                    ...
                    $("#table").on("click", "#change-status", function () {
                        ...
                        swal({
                            ...
                            if (result.value) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ route('discounts.change-status') }}",
                                    data: {
                                        'id': id,
                                    },
                                    headers: {
                                    ...
                                    success: function (response) {
                                        ...
                                        if (response.discount.is_active == 1) {
