                            ...
                            @foreach($doctors as $key=>$item)
                                <tr id="item-{{$item->id}}">
                                    ...
                                    <td>
                                        ...
                                        <a href="{{route('doctors.edit', $item->id)}}" class="edit-modal btn btn-info btn-circle btn-sm"
                                           data-info="">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- @endif --}}
                                        {{-- @if(auth()->user()->can('delete-banner')) --}}
                                        <a href="javascript:void(0)" class="delete-doctors btn btn-danger btn-circle btn-sm"
                                           data-type="{{$item->id}}">
...
@section('js_script')
            <script>
                $(document).ready( function () {
                    ...
                    $('#table').on('click','.delete-doctors',function(event){
                        event.preventDefault();
                        $object = $(this);
                        var id  = $(this).attr('data-type');
                        var url = baseUrl+"/doctors/"+id;
                        swal({
                            title: 'Are you sure?',
                            ...
                            if (result.value) {
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        ...
                                        if (response.doctor.is_active == 1) {
