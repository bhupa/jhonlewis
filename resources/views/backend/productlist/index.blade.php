                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('product-lists.create')}}"> Add Product-List lists</a></li>
                            ...
                            @foreach($productlists as $key=>$item)
                                <tr id="item-{{$item->id}}">
                                    ...
                                    <td>
                                        ...
                                        <a href="{{route('product-lists.edit', $item->slug)}}" class="edit-modal btn btn-info btn-circle btn-sm"
                                           data-info="">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- @endif --}}
                                        {{-- @if(auth()->user()->can('delete-banner')) --}}
                                        <a href="javascript:void(0)" class="delete-product-lists btn btn-danger btn-circle btn-sm"
                                           data-type="{{$item->id}}">
...
@section('js_script')
    <script>
        $(document).ready( function () {
            ...
            $('#table').on('click','.delete-product-lists',function(event){
                event.preventDefault();
                $object = $(this);
                var id  = $(this).attr('data-type');
                var url = baseUrl+"/product-lists/"+id;
                swal({
                    title: 'Are you sure?',
            ...
            $("#table").on("click", "#change-status", function () {
                ...
                swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('product-lists.change-status') }}",
                            data: {
                                'id': id,
                            },
                            headers: {
